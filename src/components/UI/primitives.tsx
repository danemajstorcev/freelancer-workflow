...
export const Button = styled.button<ButtonProps>`
  ...
  ${({ variant = 'primary', theme }) => {
    if (variant === 'primary') return css`
      ...
      color: #fff;
      &:hover { background: ${theme.accentHover}; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(99,102,241,0.3); }
...
export function projectStatusColor(status: string, theme: any): { color: string; bg: string } {
  const map: Record<string, { color: string; bg: string }> = {
...
export function priorityColor(priority: string, theme: any): { color: string; bg: string } {
  const map: Record<string, { color: string; bg: string }> = {
...
export const EmptyState = styled.div`
  ...
  .icon { font-size: 2.5rem; opacity: 0.4; }
  .title { font-size: 0.9rem; font-weight: 600; color: ${({ theme }) => theme.textSub}; }
...
export const IconBtn = styled.button`
  ...
  &:hover { color: ${({ theme }) => theme.text}; background: ${({ theme }) => theme.bgHover}; }
`;
