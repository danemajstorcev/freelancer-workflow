...
export default function TaskModal({ task, defaultProjectId, onSave, onClose }: Props) {
  const { state } = useApp();

  const [form, setForm] = useState({
    projectId: task?.projectId ?? defaultProjectId ?? (state.projects[0]?.id ?? ''),
    title:     task?.title     ?? '',
    notes:     task?.notes     ?? '',
  ...
  const handleSave = () => {
    ...
    onSave({ ...form, id: task?.id ?? uid(), createdAt: task?.createdAt ?? today() });
    onClose();
  };
      ...
      <ModalBox onClick={(e) => e.stopPropagation()}>
        ...
        <ModalBody>
          ...
          <Field>
            ...
            <Input value={form.title} onChange={(e) => set('title', e.target.value)} placeholder="e.g. Write homepage copy" />
          </Field>
