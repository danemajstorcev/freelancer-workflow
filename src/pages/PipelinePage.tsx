...
const BoardWrap = styled.div`
  ...
  @media (max-width: 1100px) { grid-template-columns: repeat(4, minmax(210px, 1fr)); }
  @media (max-width: 768px)  {
...
export default function PipelinePage() {
  ...
  const onDragOver  = (e: React.DragEvent, s: ClientStatus) => { e.preventDefault(); setOverCol(s); };
  const onDrop      = (s: ClientStatus)           => {
    if (dragId.current) dispatch({ type: 'MOVE_CLIENT', payload: { id: dragId.current, status: s } });
    setOverCol(null);
  };

  const colValue = (s: ClientStatus) =>
    state.clients.filter((c) => c.status === s).reduce((sum, c) => sum + c.value, 0);
              ...
              >
                <ColHeader>
                  <div>
                    ...
                    {clients.length > 0 && (
                      <div style={{ fontSize: '0.65rem', color: theme.textMuted, marginTop: '1px' }}>
                ...
                <ColBody>
                  {clients.length === 0 && (
                    <EmptyState style={{ padding: '1.5rem 0.5rem', minHeight: 'auto' }}>
                      <div className="sub" style={{ fontSize: '0.7rem' }}>Drop here</div>
                    </EmptyState>
                  )}
                  {clients.map((client) => (
                    <DragCard
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '7px' }}>
                        <Avatar bg={client.avatarColor} size={26}>{initials(client.name)}</Avatar>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 700, fontSize: '0.82rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{client.name}</div>
                          <div style={{ fontSize: '0.68rem', color: theme.textMuted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{client.company}</div>
                        </div>
                        <IconBtn onClick={() => setEditClient(client)} style={{ padding: '2px', flexShrink: 0 }}>✎</IconBtn>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ display: 'flex', gap: '3px', flexWrap: 'wrap' }}>
                          {client.tags.slice(0, 2).map((t) => (
                            <Badge key={t} color={theme.textMuted} bg={theme.bgTag} style={{ fontSize: '0.58rem' }}>{t}</Badge>
                          ))}
                        </div>
                        <span style={{ fontSize: '0.75rem', fontWeight: 700 }}>{formatCurrency(client.value)}</span>
                      </div>
                    </DragCard>
                  ))}
                </ColBody>
